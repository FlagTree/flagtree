...
def test_gemm_no_scf(
    ...
    assert paddle.allclose(x=c.astype(golden.dtype), y=golden, rtol=0.01, atol=0.001).item(), ""
...
@pytest.mark.parametrize(
    ...
    + [
        ...
        if not (
            epilogue == "chain-dot"
            and (shape_w_c[6] is not None or shape_w_c[1] != shape_w_c[6])
        )
    ]
    + [
        (
...
def test_gemm(
    ...
    if epilogue == "add-rows" and NUM_CTAS > 1:
        pytest.skip(
            "known failure: error getCTAsPerCGA for SliceEncodingAttr is not well-defined."
        )
    ...
    if epilogue in ["add-matrix", "add-rows", "add-cols"]:
        if TRANS_OUTPUT:
            bias = paddle.randn((N, M), device="cuda", dtype=torch_out_dtype).T
        else:
    ...
    w_order = [0, 1]
    if TRANS_OUTPUT:
        z = paddle.full((N, M), 1.0, device="cuda", dtype=torch_out_dtype).T
        z_order = [0, 1]
    else:
    ...
    assert paddle.allclose(x=z.astype(golden.dtype), y=golden, rtol=0.01, atol=0.001).item(), ""
    if is_hip():
        return
    disable_mmav3 = os.environ.get("DISABLE_MMA_V3", "not found").lower()
    if (
        disable_mmav3 not in ["on", "true", "1"]
        and BLOCK_M >= 64
        and NUM_CTAS == 1
        and BLOCK_N <= 256
    ):
        ...
        assert re.search(
            "wgmma.mma_async.sync.aligned.m\\d+n{}k16(?:.row.col)?.f32.f16.f16".format(
                wgmma_n
            ),
            ptx,
        )
