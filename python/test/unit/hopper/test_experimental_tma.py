...
@pytest.mark.skipif(not HAS_TORCH, reason = "Torch not installed — skipping tests.")
...
@pytest.mark.skipif(not HAS_PADDLE, reason = "Paddle not installed — skipping tests.")
def test_descriptor_load_ttgir_paddle():
    if (
        not paddle.device.cuda.device_count() >= 1
        or not paddle.device.cuda.get_device_capability()[0] == 9
    ):
        pytest.skip("Test requires Hopper target.")
        return
    device = "cuda"
    SIZE = 128
    x = paddle.randn(SIZE, dtype=paddle.float32, device=device)
    desc = np.empty(SIZE, dtype=np.int8)
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".ttgir") as f:
        ...
        f.flush()
        kernel = triton.compile(f.name)
    desc = paddle.tensor(desc, device=device)
    z_tri = paddle.empty_like(x)
    kernel[1, 1, 1](z_tri, desc)
    assert paddle.equal_all(x=x, y=z_tri).item()

@pytest.mark.skipif(not HAS_PADDLE, reason = "Paddle not installed — skipping tests.")
def test_experimetal_descriptor_load_paddle():
    if (
        not paddle.device.cuda.device_count() >= 1
        or not paddle.device.cuda.get_device_capability()[0] == 9
    ):
    ...
    kernel[
        1,
    ](z_tri, desc, SIZE=SIZE, num_warps=4)
    assert paddle.equal_all(x=x, y=z_tri).item()
...
def test_experimental_tma_matmul_paddle(num_stages, BLOCK_M, BLOCK_N, BLOCK_K):
    if (
        not paddle.device.cuda.device_count() >= 1
        or not paddle.device.cuda.get_device_capability()[0] == 9
    ):
    ...
    M, N, K = 8192, 8192, 1024
    paddle.seed(42)
    A = paddle.randn((M, K), dtype=paddle.float16, device=device)
    B = paddle.randn((K, N), dtype=paddle.float16, device=device)
    C = paddle.empty((M, N), dtype=paddle.float16, device=device)
    TMA_SIZE = 128
    desc_a = np.empty(TMA_SIZE, dtype=np.int8)
    desc_b = np.empty(TMA_SIZE, dtype=np.int8)
    desc_c = np.empty(TMA_SIZE, dtype=np.int8)
    ...
    desc_a = paddle.tensor(desc_a, device=device)
    desc_b = paddle.tensor(desc_b, device=device)
    desc_c = paddle.tensor(desc_c, device=device)
    ...
    ref_out = paddle.matmul(A.to(paddle.float32), B.to(paddle.float32)).to(
        paddle.float16
    )
