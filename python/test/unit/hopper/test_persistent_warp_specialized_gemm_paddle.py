    ...
    for tile_id in range(start_tile, num_tiles, NUM_SMS):
        ...
        if tile_id >= NUM_SMS:
            a_tile_ptr = tl.advance(
                a_tile_ptr, [(pid_m - pre_pid_m) * BLOCK_M, -k_tiles * BLOCK_K]
            )
            b_tile_ptr = tl.advance(
                b_tile_ptr, [-k_tiles * BLOCK_K, (pid_n - pre_pid_n) * BLOCK_N]
            )
        for k in range(0, K, BLOCK_K):
...
@pytest.mark.parametrize(
    "M,N,K,BLOCK_M,BLOCK_N,BLOCK_K,NUM_WARPS,NUM_CTAS,TRANS_A,TRANS_B,USE_TMA",
    [
        (*shape, use_tma)
        for shape in [
...
def test_user_defined_persistent_non_warp_specialized_gemm(
    M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, NUM_WARPS, NUM_CTAS, TRANS_A, TRANS_B, USE_TMA
):
    if TRANS_A:
        a = 0.1 * paddle.randn((K, M), device="cuda", dtype=paddle.float16).T
    else:
        a = 0.1 * paddle.randn((M, K), device="cuda", dtype=paddle.float16)
    if TRANS_B:
        b = 0.1 * paddle.randn((N, K), device="cuda", dtype=paddle.float16).T
    else:
        b = 0.1 * paddle.randn((K, N), device="cuda", dtype=paddle.float16)
    c = paddle.empty((M, N), device=a.place, dtype=paddle.float32)
    NUM_SMS = paddle.device.get_device_properties(
        device="gpu"
    ...
    th_c = paddle.matmul(a, b)
    if USE_TMA:
        static_persistent_tma_matmul_kernel[grid](
            a_ptr=a,
            b_ptr=b,
            c_ptr=c,
            M=M,
        ...
        static_persistent_matmul_kernel[grid](
            a_ptr=a,
            b_ptr=b,
            c_ptr=c,
            M=M,
    ...
    # assert paddle.allclose(x=th_c.astype(c.dtype), y=c, atol=0.6, rtol=0.).item(), ""
    np.testing.assert_allclose(th_c.numpy(), c.numpy(), atol=0.6, rtol=0.)
...
@pytest.mark.parametrize(
    "M,N,K,BLOCK_M,BLOCK_N,BLOCK_K,NUM_CTAS,TRANS_A,TRANS_B,USE_TMA",
    [
        (*shape, use_tma)
        for shape in [
...
def test_non_persistent_warp_specialized_gemm(
    M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, NUM_CTAS, TRANS_A, TRANS_B, USE_TMA
):
    if TRANS_A:
        a = 0.1 * paddle.randn((K, M), device="cuda", dtype=paddle.float16).T
    ...
    th_c = paddle.matmul(a, b)
    np.testing.assert_allclose(th_c, c, atol=1e-2, rtol=0.)
    # assert paddle.allclose(x=th_c, y=c, atol=1, rtol=0.).item(), ""
...
def test_user_defined_persistent_warp_specialized_gemm(
    M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, NUM_CTAS, TRANS_A, TRANS_B, USE_TMA
):
    if TRANS_A:
        a = 0.1 * paddle.randn((K, M), device="cuda", dtype=paddle.float16).T
...
def static_persistent_matmul_no_scf_kernel(
    ...
    for tile_id in range(start_tile, num_tiles, NUM_SMS):
        ...
        if FLOAT16_OUTPUT:
            c = c.to(tl.float16)
        if USE_TMA_EPILOGUE:
            c_block_ptr = tl.advance(
                c_block_ptr,
                [(pid_m - pre_pid_m) * BLOCK_M, (pid_n - pre_pid_n) * BLOCK_N],
            )
...
def test_static_persistent_matmul_no_scf_kernel(
    M,
    N,
    K,
    NUM_CTAS,
    ...
    if TRANS_A:
        a = paddle.randn((K, M), device="cuda", dtype=paddle.float16).T
    else:
        a = paddle.randn((M, K), device="cuda", dtype=paddle.float16)
    if TRANS_B:
        b = paddle.randn((N, K), device="cuda", dtype=paddle.float16).T
    else:
        b = paddle.randn((K, N), device="cuda", dtype=paddle.float16)
    if OUTPUT_TYPE == "float16":
        c = paddle.empty((M, N), device=a.place, dtype=paddle.float16)
    else:
        c = paddle.empty((M, N), device=a.place, dtype=paddle.float32)
    NUM_SMS = paddle.device.get_device_properties(
        device="gpu"
    ...
    static_persistent_matmul_no_scf_kernel[NUM_SMS,](
        ...
        stride_cm=c.strides[0],
        stride_cn=c.strides[1],
        BLOCK_M=M if M < 128 else M // 2,
        BLOCK_N=N if N < 128 else N // 2,
        BLOCK_K=K,
        NUM_SMS=NUM_SMS,
        num_warps=NUM_WARPS,
        num_ctas=NUM_CTAS,
    ...
    a_f32 = a.to(paddle.float32)
    b_f32 = b.to(paddle.float32)
    golden = paddle.matmul(a_f32, b_f32)
...
@pytest.mark.parametrize(
    ...
    + [
        ...
        if not (
            epilogue == "chain-dot"
            and (shape_w_c[5] is not None or shape_w_c[0] != shape_w_c[1])
        )
    ]
    + [
        (
    ...
    + [
        ...
        for shape in [[512, 360, 1024], [360, 4096, 512]]
        for out_dtype in ["float32"]
        for use_tma_store in [False, True]
...
def test_full_static_persistent_matmul_kernel(
    BLOCK_M,
    BLOCK_N,
    BLOCK_K,
    NUM_WARPS,
    ...
    ) in ["128-128-128-4-1-256-256-192-none-float32-True-3"]:
        pytest.skip("out of resource: shared memory, Required: 263168")
    if "-".join(
        map(
            str,
            [BLOCK_M, BLOCK_N, BLOCK_K, NUM_WARPS, NUM_CTAS, M, N, K, TRANS_A, TRANS_B],
        )
    ) in [
    ...
    if "-".join(
        map(str, [BLOCK_M, BLOCK_N, BLOCK_K, NUM_WARPS, NUM_CTAS, M, N, K, TRANS_B])
    ) in [
    ...
    if epilogue == "chain-dot":
        pytest.skip(
            "known failure: Assertion !region.empty() && unexpected empty region."
        )
    ...
    if out_dtype == "float16" and epilogue != "softmax":
        ...
        torch_out_dtype = paddle.float16
    else:
        out_dtype = tl.float32
        torch_out_dtype = paddle.float32
    if epilogue in ["add-matrix", "add-rows", "add-cols"]:
        bias = paddle.randn((M, N), device="cuda", dtype=torch_out_dtype)
    else:
        bias = paddle.randn((1, 1), device="cuda", dtype=torch_out_dtype)
    if epilogue == "chain-dot":
        w = paddle.randn((N, N), device="cuda", dtype=paddle.float16).T
    else:
        w = paddle.randn((1, 1), device="cuda", dtype=paddle.float16).T
    z = paddle.full((M, N), 1.0, device="cuda", dtype=torch_out_dtype)
    a_f32 = a.to(paddle.float32)
    b_f32 = b.to(paddle.float32)
    dot = paddle.matmul(a_f32, b_f32)
    ...
    def process_epilogue(d, bias, w, epilogue):
        if epilogue == "add-matrix":
            ...
            num = paddle.exp(x=d - paddle.compat.max(d, dim=-1, keepdim=True)[0])
            denom = paddle.sum(num, dim=-1, keepdim=True)
            ref = num / denom
        elif epilogue == "chain-dot":
            ref = paddle.matmul(d, w.to(paddle.float32))
        else:
            ref = d
        return ref
    ...
    if NUM_CTAS > 1:
        src = triton.compiler.ASTSource(
            fn=empty_kernel, signature="i32", constants={"BLOCK_M": 64, "BLOCK_N": 64}
        )
        null_kernel = triton.compile(src)
        null_kernel._init_handles()
        device = int(driver.active.get_current_device().split(":")[1])
        max_shared_mem = driver.active.utils.get_device_properties(device)[
            "max_shared_mem"
        ]
        NUM_SMS = driver.active.utils.cuOccupancyMaxActiveClusters(
            null_kernel.function, max_shared_mem, NUM_CTAS, 1, 1
        )
    ...
    full_static_persistent_matmul_kernel[grid](
        ...
        stride_bk=b.strides[0],
        stride_bn=b.strides[1],
        stride_wm=w.strides[0],
        stride_wn=w.strides[1],
        stride_zm=z.strides[0],
        stride_zn=z.strides[1],
        BLOCK_M=BLOCK_M,
        BLOCK_N=BLOCK_N,
        BLOCK_K=BLOCK_K,
        GROUP_SIZE_M=8,
    ...
    paddle.set_printoptions(precision=4, threshold=1000000, edgeitems=3, linewidth=80)
    golden = paddle.nn.functional.normalize(x=golden)
    z = paddle.nn.functional.normalize(x=z)
    np.testing.assert_allclose(z.numpy(), golden.numpy(), atol=0.2, rtol=0.001 )
