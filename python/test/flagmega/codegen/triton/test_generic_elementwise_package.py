    ...
    class Graph(fm.Module):
        ...
        def forward(self):
            ...
            if op in {"math.add", "math.mul"}:
                ...
                output = getattr(fm.F.math, op.rsplit(".", 1)[-1])(
                    lhs, rhs, name="output"
                )
                inputs = (lhs, rhs)
            elif op == "math.silu":
...
def test_single_kernel_package_resolves_selected_tir_family_variant_template(
    tmp_path, op, variant,
):
    ...
    call = next(
        value
        for value in package["runtime_binding"]["call_abi"]["kernel_calls"]
        if value["semantic_op"] == op
    )
    assert call["semantic_op"] == op
    assert call["variant"] == variant
    assert {"kernel": "elementwise", "variant": variant} in package[
