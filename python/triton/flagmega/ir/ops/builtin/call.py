...
"""Typed high-level function calls.

Calls are introduced by importers and survive every middle-end stage.  The
callee therefore remains visible in editable Python checkpoints instead of
being rediscovered from a flattened graph by code generation.
"""
...
class Call(OpDefinition):
    ...
    def normalize_attrs(cls, attributes: Mapping[str, object]) -> dict[str, object]:
        ...
        if not required.issubset(attributes) or not set(attributes).issubset(allowed):
            raise IRSchemaError(
                "F.builtin.call requires result_type and callee, with optional effect."
            )
        ...
        if not isinstance(result_type, IRType):
            raise IRSchemaError("F.builtin.call result_type must be an IRType.")
        if not callee:
            raise IRSchemaError("F.builtin.call callee must be non-empty.")
        if not isinstance(effect, Effect):
            raise IRSchemaError("F.builtin.call effect must be an Effect.")
    ...
    def verify(cls, node: Node, module: IRModule) -> None:
        ...
        if set(node.attrs) != {"callee"} or not str(node.attrs["callee"]):
            raise IRVerificationError(
                "builtin.call requires exactly one non-empty callee attribute.",
                node_id=node.id,
            )
...
def function_result_type(module: IRModule, function_name: str) -> IRType:
    """Return the value type exposed by ``function_name`` at a call site."""
