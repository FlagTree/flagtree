...
"""Compile-time scalar values used by semantic TIR call sites."""
...
class ScalarConst(OpDefinition):
    ...
    def normalize_attrs(cls, attributes: Mapping[str, object]) -> dict[str, object]:
        ...
        if not isinstance(result_type, TensorType) or result_type.rank != 0:
            raise IRSchemaError("F.tir.scalar_const requires a rank-zero TensorType.")
        if not isinstance(value, (bool, int, float)):
            raise IRSchemaError("F.tir.scalar_const value must be bool or numeric.")
    ...
    def verify(cls, node: Node, module: IRModule) -> None:
        ...
        if set(node.attrs) != {"value"} or not isinstance(node.type, TensorType) or node.type.rank != 0:
            raise IRVerificationError(
                "tir.scalar_const requires one value and a rank-zero tensor type.",
                node_id=node.id,
            )
