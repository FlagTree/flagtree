...
def _device_capability(device=None):
    if HAS_TORCH:
        return torch.cuda.get_device_capability(device)
    else:
        # paddle returns (major, minor)
...
def _current_device():
    if HAS_TORCH:
        ...
        # Paddle doesn't expose index in the same way; Triton driver works with 0
        return 0

def _element_size_of_dtype(dtype):
    if HAS_TORCH:
        return torch.tensor([], dtype=dtype).element_size()
    # paddle mapping
    mapping = {
        getattr(paddle, "float16"): 2,
        getattr(paddle, "bfloat16", object()): 2,  # if exists
        paddle.float32: 4,
        paddle.float64: 8,
        paddle.int16: 2,
        paddle.int32: 4,
        paddle.int64: 8,
        getattr(paddle, "uint8", object()): 1,
        getattr(paddle, "int8", object()): 1,
        paddle.bool: 1,
    }
    return mapping[dtype]

def _is_float32(dtype):
    if HAS_TORCH:
        return dtype == torch.float32
    else:
        return dtype == paddle.float32

def _is_fp16(dtype):
    if HAS_TORCH:
        return dtype == torch.float16
    else:
        return dtype == paddle.float16
...
def get_tflops(device, num_ctas, num_warps, dtype):
    ...
    if capability[0] < 8 and _is_float32(dtype):
        return get_simd_tflops(device, num_ctas, num_warps, dtype)
    return get_tensorcore_tflops(device, num_ctas, num_warps, dtype)
...
def estimate_matmul_time(
    ...
    dtype = A.dtype
    dtsize = _element_size_of_dtype(dtype)
...
def early_config_prune(configs, named_args, **kwargs):
    ...
    # BLOCK_M, BLOCK_N, BLOCK_K, SPLIT_K, num_warps, num_stages
    dtsize = _element_size_of_dtype(named_args['A'].dtype)
    dtype = named_args['A'].dtype
