...
from triton.runtime.driver import flagtree_backend_specialization
@autotune(
    configs=flagtree_backend_specialization("get_nv_configs") + get_configs_io_bound() + flagtree_backend_specialization("get_configs_compute_bound"),
    key=['M', 'N', 'K'],
    prune_configs_by={
        'early_config_prune': early_config_prune,
        'perf_model': estimate_matmul_time,
...
@jit
def _kernel_iluvatar(A, B, C, M, N, K,  #
                    stride_am, stride_ak,  #
                    stride_bk, stride_bn,  #
                    stride_cm, stride_cn,  #
                    acc_dtype: tl.constexpr,  #
...
class _matmul(torch.autograd.Function):
    from triton.runtime.driver import flagtree_backend_specialization
    if flagtree_backend_specialization("is_iluvatar"):
        kernel = _kernel_iluvatar
    else:
        kernel = _kernel
