...
if HAS_TORCH:
    ...
    class _attention(paddle.autograd.PyLayer):
        ...
        def forward(ctx, q, k, v, causal, sm_scale, sequence_parallel=False):
            ...
            grid = (cdiv(q.shape[2], BLOCK_M), q.shape[0] * q.shape[1], 1)
            L = paddle.empty((q.shape[0] * q.shape[1], q.shape[2]), dtype=paddle.float32)
            num_warps = 4 if Lk <= 64 else 8
            _fwd_kernel[grid](
                q, k, v, sm_scale,  #
                L,  #
        ...
        def backward(ctx, do):
            ...
            q, k, v, o, L = ctx.saved_tensor()
            sequence_parallel = ctx.sequence_parallel
            seq_len_kv = k.shape[2]
            # do = do.contiguous()
            if sequence_parallel:
                replicas = cdiv(seq_len_kv, BLOCK)
                new_dq_shape = (replicas,) + tuple(q.shape)
                dq = paddle.zeros(new_dq_shape, dtype=q.dtype)
            else:
                dq = paddle.zeros_like(q, dtype=q.dtype)
