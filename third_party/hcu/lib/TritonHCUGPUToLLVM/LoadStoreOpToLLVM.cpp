...
Value redundantDataMask(Type valueTy, ConversionPatternRewriter &rewriter,
                        Location loc, const HCU::TargetInfo &targetInfo) {
...
struct LoadOpConversion : public ConvertOpToLLVMPattern<triton::LoadOp>,
  ...
  matchAndRewrite(triton::LoadOp op, OpAdaptor adaptor,
    ...
    // Get the LLVM values for mask same as before
    SmallVector<Value> maskElems =
...
struct BufferLoadOpConversion
  ...
  using ConvertOpToLLVMPattern<
      triton::hcugpu::BufferLoadOp>::ConvertOpToLLVMPattern;

  BufferLoadOpConversion(LLVMTypeConverter &converter,
                         const HCU::TargetInfo &targetInfo,
                         ModuleAxisInfoAnalysis &axisAnalysisPass,
                         PatternBenefit benefit)
      : ConvertOpToLLVMPattern<triton::hcugpu::BufferLoadOp>(converter,
                                                             benefit),
        LoadStoreConversionBase(targetInfo, axisAnalysisPass) {}
...
struct BufferStoreOpConversion
    : public ConvertOpToLLVMPattern<triton::hcugpu::BufferStoreOp>,
      public LoadStoreConversionBase {
  using ConvertOpToLLVMPattern<
      triton::hcugpu::BufferStoreOp>::ConvertOpToLLVMPattern;

  BufferStoreOpConversion(LLVMTypeConverter &converter,
                          const HCU::TargetInfo &targetInfo,
                          ModuleAxisInfoAnalysis &axisAnalysisPass,
                          PatternBenefit benefit)
      : ConvertOpToLLVMPattern<triton::hcugpu::BufferStoreOp>(converter,
                                                              benefit),
        LoadStoreConversionBase(targetInfo, axisAnalysisPass) {}
  ...
  matchAndRewrite(triton::AtomicRMWOp op, OpAdaptor adaptor,
    ...
    if ((useAtomicOps && f32v4) || (useAtomicOps && f64v2)) { 
      ...
      for (size_t i = 0; i < elemsPerThread; i += vec) {
        ...
        Value Atomicval = undef(vecTy);
        for (size_t s = 0; s < vec; ++s) {
          Value otherElem = valElements[i + s];
          Value indexVal = createIndexAttrConstant(
              rewriter, loc, this->getTypeConverter()->getIndexType(), s);
...
struct AsyncCopyGlobalToLocalOpConversion
  ...
  matchAndRewrite(triton::gpu::AsyncCopyGlobalToLocalOp op, OpAdaptor adaptor,
    ...
    auto srcLyt = srcTy.getEncoding();
    auto resTy = op.getResult().getType();
    auto resLyt = resTy.getEncoding();
    auto resElemTy = getTypeConverter()->convertType(resTy.getElementType());
    assert(!isTensorPointerType(srcTy) &&
           "Cannot convert load with a tensor pointer into LLVM; "
           "this case should be transformed to normal load before lowering");
    assert((isa<BlockedEncodingAttr, SliceEncodingAttr>(srcLyt) &&
    ...
    Value llMask = adaptor.getMask();
    Value llOther = adaptor.getOther();
    Value llRes = adaptor.getResult();

    // Determine the vectorization size
    Type valueElemTy = resElemTy;
    unsigned vec = getVectorSize(src);
    if (op.getIsSwizzle())
      vec = op.getVec();
    ...
    // Get the LLVM values for pointers
    auto ptrElems = unpackLLElements(loc, llSrc, rewriter);
    assert(ptrElems.size() == numElems);
    ...
    // compute shared offset
    auto memDescSubviewOp = result.getDefiningOp<MemDescSubviewOp>();
    SmallVector<Value> MDSOffsetVals = memDescSubviewOp.getOffsets();
    ...
    Value warpId;
    {
      ...
      Value threadsPerWarp = i32_val(warpSize);
      Value vWarpId = udiv(threadId, threadsPerWarp);
      GCNBuilder builder;
      auto v_firstlane = *builder.create("v_readfirstlane_b32");
    ...
    Value zero = rewriter.create<LLVM::ConstantOp>(
        ...
        rewriter.getI32IntegerAttr(numVecs));
...
struct AsyncWaitOpConversion
  ...
  matchAndRewrite(triton::gpu::AsyncWaitOp op, OpAdaptor adaptor,
    ...
    oss << "s_waitcnt vmcnt(" << num << ")";
    {
      GCNBuilder builder;
      auto s_wait = *builder.create(oss.str());
      s_wait();
      builder.launch(rewriter, op.getLoc(), void_ty(rewriter.getContext()));
    }
    {
      GCNBuilder builder;
      auto s_barrier = *builder.create("s_barrier");
      s_barrier();
      builder.launch(rewriter, op.getLoc(), void_ty(rewriter.getContext()));
    }
    rewriter.eraseOp(op);
