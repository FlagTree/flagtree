...
using triton::AddPtrOp;
    ...
    bool getNoSwizzleMask(mlir::arith::AndIOp rowColMaskDef, bool dstRow) {
        ...
        if (auto rMaskOp = dyn_cast<triton::BroadcastOp>(rMaskDef)) {
            auto rMaskShape  = cast<RankedTensorType>(rMaskOp.getSrc().getType()).getShape();
        ...
        for (Operation* op : slice) {
            ...
            if (auto cmpTy = dyn_cast<RankedTensorType>(val.getType())) {
                ...
                if (auto sliceLy = dyn_cast<SliceEncodingAttr>(cmpLy))
                    val = rewriter.create<triton::ExpandDimsOp>(loc, val, sliceLy.getDim());
            }
            auto cmpTy = getNewType(val.getType(), inLy);
            val = rewriter.create<ConvertLayoutOp>(loc, cmpTy, val);
            unsigned int bitWidth = cast<RankedTensorType>(colOff.getType()).getElementType().getIntOrFloatBitWidth();
            if (bitWidth == 64) {
                auto colOffShape = dyn_cast<RankedTensorType>(val.getType()).getShape();
                auto colOffEncoding = dyn_cast<RankedTensorType>(val.getType()).getEncoding();
                auto newRowType = RankedTensorType::get(colOffShape, rewriter.getI64Type(), colOffEncoding);
                val = rewriter.create<arith::ExtSIOp>(loc, newRowType, val);
            }
            auto cVal = rewriter.create<triton::BroadcastOp>(loc, colOff.getType(), val);
            predToVal.push_back(std::make_pair(cmpOp.getPredicate(), cVal.getResult()));
            ...
            if (auto subTy = dyn_cast<RankedTensorType>(subOp.getLhs().getType())) {
                ...
                if (auto sliceLy = dyn_cast<SliceEncodingAttr>(subLy))
                    val = rewriter.create<triton::ExpandDimsOp>(loc, addOp.getLhs(), sliceLy.getDim());
    ...
    for (auto kv : predToVal) {
        Operation *op = rewriter.create<mlir::arith::CmpIOp>(loc, kv.first, colM, kv.second);
    ...
    Value changeMask(Location loc, mlir::PatternRewriter &rewriter,
                        Value mask, Value colOff, RankedTensorType inTy, bool dstRow) {
        ...
        if (isa<mlir::arith::ConstantOp, triton::SplatOp>(currentOp)) {
            ...
            return (naiveLy.getSizePerThread() == inLy.getSizePerThread() &&
                    ...
                    naiveLy.getOrder() == inLy.getOrder()) ? mask : rewriter.create<ConvertLayoutOp>(loc, inTy, mask);
        } else {
            auto maskDef = dyn_cast<mlir::arith::AndIOp>(currentOp);
            auto rowColMask = maskDef.getRhs();
            auto offSetMask = maskDef.getLhs();
            auto newOffsetMask = rewriter.create<ConvertLayoutOp>(loc, inTy, offSetMask);
            if (auto arg = dyn_cast<BlockArgument>(rowColMask)) {
                auto newRowColMask = rewriter.create<ConvertLayoutOp>(loc, inTy, rowColMask);
                return rewriter.create<mlir::arith::AndIOp>(loc, newOffsetMask, newRowColMask);
            }
            ...
            if (isa<mlir::arith::AndIOp>(rowColDef)) {
                ...
                auto rowMask = rowColMaskDim ? rowColMaskDef.getRhs() : rowColMaskDef.getLhs(); 
                auto colMask = rowColMaskDim ? rowColMaskDef.getLhs() : rowColMaskDef.getRhs(); 

                auto rowMaskType = dyn_cast<RankedTensorType>(rowMask.getType());
                Value newRowMask = rowMaskType.getEncoding() == inTy.getEncoding() ?
                        rowMask : rewriter.create<ConvertLayoutOp>(loc, inTy, rowMask);                

                auto swizzleMask = colMask;
                auto newColMask = changeMaskInner(loc, rewriter, swizzleMask, colOff, inLy);
                auto newMask = rewriter.create<mlir::arith::AndIOp>(loc, newRowMask, newColMask);
                newMask = rewriter.create<mlir::arith::AndIOp>(loc, newOffsetMask, newMask);
                ...
                if (isa<BroadcastOp>(rowColDef)) {
                    ...
                    auto rowOrColShape = dyn_cast<RankedTensorType>(rowOrColValue.getType()).getShape();
                    if ((dstRow && rowOrColShape[0] == 1) || (!dstRow && rowOrColShape[1] == 1)) {
                        auto newColMask = changeMaskInner(loc, rewriter, rowOrColValue, colOff, inLy);
                        auto newMask = rewriter.create<mlir::arith::AndIOp>(loc, newOffsetMask, newColMask);
                        return newMask;
                    }
                }
                //直接返回原来的mask
    ...
    void applySwizzle(Location loc, mlir::Operation *op, mlir::PatternRewriter &rewriter,
                            RankedTensorType ppType, RankedTensorType newTensorType,
                            Value row, Value col, Value rowOff, Value basePtr, triton::AddPtrOp addptr,
                            unsigned int outVec, unsigned int perPhase, unsigned int maxPhase,
                            bool isConvert, bool dstRow, unsigned int inVec, unsigned int bitWidth) {
        SetVector<Operation*> slice;
        mlir::BackwardSliceOptions opt;
        opt.omitBlockArguments = true;
        opt.filter = nullptr;
        ...
        if (isLoadOrWhere && makeRange) {
            ...
            row = rewriter.create<tt::ExpandDimsOp>(loc, makeRange->getResult(0), axisVal);
        }
        ...
        if (bitWidth == 64) {
            ...
            auto newRowType = RankedTensorType::get(rowShape, rewriter.getI64Type(), rowEncoding);
            auto newColType = RankedTensorType::get(colShape, rewriter.getI64Type(), rowEncoding);
            row = rewriter.create<arith::ExtSIOp>(loc, newRowType, row);
            col = rewriter.create<arith::ExtSIOp>(loc, newColType, col);
        }
        auto ppTypeR = getNewType(dyn_cast<RankedTensorType>(row.getType()), ppType.getEncoding());
        auto ppTypeC = getNewType(dyn_cast<RankedTensorType>(col.getType()), ppType.getEncoding());

        auto rowb = rewriter.create<triton::BroadcastOp>(loc, ppType, (isConvert ?
            rewriter.create<ConvertLayoutOp>(loc, ppTypeR, row) : row));
        auto ccol = rewriter.create<triton::BroadcastOp>(loc, ppType, (isConvert ?
            rewriter.create<ConvertLayoutOp>(loc, ppTypeC, col) : col));

        auto outVecConst = rewriter.create<mlir::arith::ConstantIntOp>(loc, outVec, bitWidth);
        auto outVecConstant = rewriter.create<triton::SplatOp>(loc, ppType, outVecConst);

        auto phaseConst = rewriter.create<mlir::arith::ConstantIntOp>(loc, perPhase, bitWidth);
        auto maxPhaseConst = rewriter.create<mlir::arith::ConstantIntOp>(loc, maxPhase, bitWidth);
        auto splatPhase = rewriter.create<triton::SplatOp>(loc, ppType, phaseConst);
        auto splatMaxPhase = rewriter.create<triton::SplatOp>(loc, ppType, maxPhaseConst);
        auto rowPhase = rewriter.create<mlir::arith::RemUIOp>(loc,
                                rewriter.create<mlir::arith::DivUIOp>(loc, rowb, splatPhase), splatMaxPhase);

        auto coloff = rewriter.create<mlir::arith::XOrIOp>(loc,
                                    rewriter.create<mlir::arith::DivUIOp>(loc, ccol, outVecConstant), rowPhase);
        auto colOffSwizzled = rewriter.create<mlir::arith::MulIOp>(loc, coloff, outVecConstant);

        auto colOffInVec = rewriter.create<mlir::arith::RemUIOp>(loc, ccol, outVecConstant);
        auto colFinalOff = rewriter.create<mlir::arith::AddIOp>(loc, colOffSwizzled, colOffInVec);

        auto newOffset = rewriter.create<mlir::arith::AddIOp>(loc,(isConvert ?
                  rewriter.create<ConvertLayoutOp>(loc, ppType, rowOff) :  rowOff), colFinalOff);
        auto newOffsetVal = bitWidth == 32 ? newOffset.getResult():
                                rewriter.create<mlir::arith::TruncIOp>(loc, newTensorType, newOffset);

        auto oldMask = copyOp.getMask();
        if (oldMask) {
            auto maskType = getNewType(dyn_cast<RankedTensorType>(copyOp.getMask().getType()), newTensorType.getEncoding());
            auto newMask = changeMask(loc, rewriter, oldMask, colFinalOff, maskType, dstRow);
        ...
        Operation *newCopy;
        if (!isConvert) {
            newAddPtr = rewriter.create<triton::AddPtrOp>(loc, addptr.getType(), basePtr, newOffsetVal);
            newCopy = rewriter.create<AsyncCopyGlobalToLocalOp>(loc, newAddPtr, copyOp.getResult(),
                copyOp.getMask(), copyOp.getOther(),
                copyOp.getCache(), copyOp.getEvict(), copyOp.getIsVolatile(), true, inVec);
        } else {

            auto defPtrOp = basePtr.getDefiningOp();
            auto splatop = dyn_cast<triton::SplatOp>(defPtrOp);
            auto srcPtr = splatop.getSrc();
            auto newPtrType = RankedTensorType::get(ppType.getShape(), srcPtr.getType(), ppType.getEncoding());
            auto newSplatPtr = rewriter.create<triton::SplatOp>(loc, newPtrType, srcPtr);
            newAddPtr = rewriter.create<triton::AddPtrOp>(loc, newPtrType, newSplatPtr, newOffsetVal);
            Value newOther;
            if (copyOp.getOther()) {
                auto otherType = getNewType(dyn_cast<RankedTensorType>(copyOp.getOther().getType()), newTensorType.getEncoding());
                auto newOther = rewriter.create<ConvertLayoutOp>(loc, otherType, copyOp.getOther());
            }
            newCopy = rewriter.create<AsyncCopyGlobalToLocalOp>(loc, newAddPtr, copyOp.getResult(),
                    copyOp.getMask(), newOther,
                copyOp.getCache(), copyOp.getEvict(), copyOp.getIsVolatile(), true, outVec);
        }
...
public:
    GlobalToLocalSwizzle(mlir::MLIRContext *context)
        : mlir::RewritePattern(ttg::AsyncCopyGlobalToLocalOp::getOperationName(), 1, context) {}
    ...
    matchAndRewrite(mlir::Operation *op,
        ...
        unsigned inVec = triton::gpu::getUniqueContigPerThread(srcLayout, srcShape)[dstOrder[0]];
        ...
        auto backSlice = 
            [&](Value offset, SetVector<Operation *> &slice) {
        ...
        auto rowOffType = cast<RankedTensorType>(rowOff.getType()).getElementType();  //i32
        ...
        if (inVec <= outVec) {
            ...
            newTensorType = RankedTensorType::get(rowShape, rewriter.getI32Type(),
                                                                rowSlice2d);
            applySwizzle(loc, op, rewriter, ppType, newTensorType, row, col, rowOff, basePtr, addptr,
                        outVec, perPhase, maxPhase, false, dstRow, inVec, bitWidth);

        } else {
            SmallVector<unsigned> sizePerThread(srcTy.getRank(), 1);
            sizePerThread[srcOrder[0]] = outVec;
            auto warpsPerCTA = dyn_cast<BlockedEncodingAttr>(rowSlice2d).getWarpsPerCTA();
            auto numWarps = accumulate(warpsPerCTA.begin(), warpsPerCTA.end(), 1, std::multiplies<int>());
            ...
            newTensorType = RankedTensorType::get(rowShape, rewriter.getI32Type(), newLayout);

            applySwizzle(loc, op, rewriter, ppType, newTensorType, row, col, rowOff, basePtr, addptr,
                        outVec, perPhase, maxPhase, true, dstRow, inVec, bitWidth);
