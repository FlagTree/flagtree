...
static void addParamForBlockRet(RewriterBase &rewriter, scf::ForOp forOp,
                SmallVector<Operation*>& loadIner, SmallVector<Operation*> &outerMemorySub) {
...
static void insertWaitNum(RewriterBase &rewriter, scf::ForOp forOp, llvm::MapVector<Operation* , Operation*> &localLoadToMemSub,
                  llvm::MapVector<Operation*, Value>& memSubToToken, llvm::MapVector<Value, int>& tokenToArgNum) {
    ...
    for (auto localNum : localToNum) {
      ...
      if (memSubToToken.find(mem) != memSubToToken.end()) {
        ...
        auto wait = rewriter.create<ttg::AsyncWaitOp>(forOp.getLoc(), argToken, localNum.second);
...
class TritonHCUFaFwdWaitPass
    : public TritonHCUFaFwdWaitBase<
        TritonHCUFaFwdWaitPass> {
public:
    ...
    void runOnOperation() override {
        ...
        for (scf::ForOp forOp : loops) {
            ...
            for (auto memOp : outerMemorySub) {
                for (auto op : memOp->getUsers()) {
                    ...
                    for (auto commOp : op->getUsers()) {
                        if (isa<ttg::AsyncCommitGroupOp>(commOp) &&
                              (forOp.getOperation()->getBlock() != commOp->getBlock() || commOp->isBeforeInBlock(forOp))) {
            ...
            scf::ForOp newForOp = replaceForOpWithNewSignature(builder, forOp, commGroup);
            forOp.erase();
            forOp = newForOp;
            addParamForBlockRet(builder, forOp, loadIner, outerMemorySub);
            insertWaitNum(builder, forOp, localLoadToMemSub, memSubToToken, tokenToArgNum);
