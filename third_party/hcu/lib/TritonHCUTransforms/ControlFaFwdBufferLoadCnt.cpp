...
//获取每个AsyncGlobalToLocal的group ,让他们作为forOp的参数传入 才能在 forOp里进行asyn_wait修改
...
static bool controlBufferVmcnt(Block* block) {
    ...
    for (Operation *op : dotOperands) {
      ...
      if (isa<BlockArgument>(loadOperand)) {
        ...
        if (blockArg && blockArg.getOwner() == *(block->getPredecessors().begin())) {
...
class TritonHCUFaFwdControlCntPass
    : public TritonHCUFaFwdControlCntBase<
        TritonHCUFaFwdControlCntPass> {
public:
    TritonHCUFaFwdControlCntPass() = default;
    TritonHCUFaFwdControlCntPass(int cnt_control) { this->cnt_control = cnt_control; }
...
std::unique_ptr<Pass> mlir::createTritonHCUFaFwdControlCntPass(int cnt_control) {
