...
void collectDependentOps(Operation *op, std::vector<Operation*> &dependentOps) {
  ...
  void moveOpWithDependencies(Operation *op, Operation *target, std::set<Operation*> &moveOps) {
  ...
  // 定义比较函数
  static bool compareMaps(const std::map<Operation*, std::pair<Operation*, int>>& map1,
                  const std::map<Operation*, std::pair<Operation*, int>>& map2) {
      // 找到map1和map2中的最大int值
      int max1 = std::max_element(map1.begin(), map1.end(),
          [](const std::pair<Operation*, std::pair<Operation*, int>>& p1,
            const std::pair<Operation*, std::pair<Operation*, int>>& p2) {
              return p1.second.second < p2.second.second;
          })->second.second;

      int max2 = std::max_element(map2.begin(), map2.end(),
          [](const std::pair<Operation*, std::pair<Operation*, int>>& p1,
            const std::pair<Operation*, std::pair<Operation*, int>>& p2) {
              return p1.second.second < p2.second.second;
          })->second.second;
        ...
        mod.walk([&](Operation *op) {
            ...
            if (isa<cf::CondBranchOp>(op)) {
                ...
                block->walk([&](Operation *op){
                    ...
                    if (isa<triton::LoadOp, triton::hcugpu::BufferLoadOp, triton::gpu::AsyncCopyGlobalToLocalOp>(op)){
                        loads.push_back(op);
                ...
                loads.erase(std::remove_if(loads.begin(), loads.end(), 
                    [this, dots, &index, loads_num, dots_num](Operation* load) {  
                        if (dots.size() == 1){
                ...
                // 判断有依赖就不移动
                loads.erase(std::remove_if(loads.begin(), loads.end(), [this, dots](Operation* load) {
                    bool flag = false;
                    for (auto dot : dots){
                ...
                /*
                    ...
                    通过 dot 两个参数中的形状来判断是否是与 dot 指令直接相关的 load 操作。
                    (反量化因子相关load指令, 目前只有 loads 里面仅有scales的时候， 把scales相关的load去除掉，如果既有 dot load 又有 scales load 那就不消除)
                */
                // if(loads.size() != 4)
                    loads.erase(std::remove_if(loads.begin(), loads.end(), [this, dots](Operation* load) {
                        bool flag = true;
                        for(auto op : dots){
                            if(auto dotOp = dyn_cast<triton::DotOp>(op)){
                                ...
                                } else if (auto bufferLoadOp = dyn_cast<triton::hcugpu::BufferLoadOp>(load)) {
                                    loadValueTy = bufferLoadOp.getType();
                                } else if (auto asyncCopyOp = dyn_cast<triton::gpu::AsyncCopyGlobalToLocalOp>(load)) {
                                    loadValueTy = asyncCopyOp.getType();  // todo 这里没测试过
                                } else {
                                    continue;  // 如果不是以上任何类型，跳过
                                }
                                if (auto tensorType = dyn_cast<RankedTensorType>(loadValueTy)) {
                                    // load type, shape
                                    Type loadElementType = tensorType.getElementType();
                                    ArrayRef<int64_t> loadShape = tensorType.getShape();
                                    auto getElementBitWidth = [](Type elementType) -> std::optional<unsigned> {
                                    ...
                                    if((getElementBitWidth(loadElementType) == getElementBitWidth(aElementType) && loadShape == aShape) 
                                        || (getElementBitWidth(loadElementType) == getElementBitWidth(bElementType) && loadShape == bShape))
                                        flag = false;
                                }
                            }
                        }
                ...
                // 判断 loadOp 的操作数是否与 dotOp 有依赖关系，如果没有依赖关系的话，就可以将 loadOp 相关的代码移动到 dot 前面
