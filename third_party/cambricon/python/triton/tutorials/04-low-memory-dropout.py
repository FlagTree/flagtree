...
if __name__ == "__main__":
    # Input tensor
    x = torch.randn(size=(10, )).to("mlu")
    # Dropout mask
    p = 0.5
    x_keep = (torch.rand(size=(10, )) > p).to(torch.int32).to("mlu")
    #
...
if __name__ == "__main__":
    ...
    print(
        tabulate.tabulate([["input"] + x.tolist(), ["output (seed = 123)"] + output.tolist(),
                           ["output (seed = 123)"] + output2.tolist(), ["output (seed = 512)"] + output3.tolist()]))

# %%
# Et Voilà! We have a triton kernel that applies the same dropout mask provided the seed is the same!
# If you'd like explore further applications of pseudorandomness in MLU programming, we encourage you
