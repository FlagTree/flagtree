...
@core.builtin
@_check_dtype(dtypes=["int32", "fp32"])
@_add_math_1arg_docstr("pow")
@core._tensor_member_fn
def pow(x, y, _builder=None):
    x = semantic.to_tensor(x, _builder)
    y = semantic.to_tensor(y, _builder)
    x, y = semantic.broadcast_impl_value(x, y, _builder)
    dst_dtype = semantic.computation_type_impl(x.dtype, False, y.dtype, False, False, False)
    if x.dtype.is_int32():
        x = semantic.cast(x, core.float32, _builder)
    out_dtype = x.type
    if dst_dtype == "int32":
        out_dtype = y.type
    return core.tensor(_builder.create_pow(x.handle, y.handle), out_dtype)
