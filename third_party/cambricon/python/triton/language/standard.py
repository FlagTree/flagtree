...
def empty_like(input):
    """
    Creates an uninitialized tensor with the same shape as a given tensor.
    """
    return core.empty(input.shape, input.dtype)
...
def max(input, axis=None, return_indices=False, return_indices_tie_break_left=True, keep_dims=False):
    ...
    if return_indices:
        ...
        if core.constexpr(input.dtype.primitive_bitwidth) < core.constexpr(32):
            # Cambricon specific.
            if not core.constexpr(input.dtype.is_floating()):
...
@core._tensor_member_fn
@core.builtin
def flip(input, dim, _builder=None):
    """
    ...
    """
    # Return input as-is if it is a scalar tensor
    if len(input.shape) == 0:
        return input

    input = semantic.to_tensor(input, _builder)
    dim = core._shape_check_impl(dim)
    ...
    normalized_dim = []
    for d in dim:
        if d < -total_dims or d >= total_dims:
            raise ValueError(
                f"Dimension out of range (expected to be in range of [{-total_dims}, {total_dims - 1}], but got: {d})")
        # Convert negative dimensions to positive indices
        if d < 0:
            d += total_dims
        normalized_dim.append(d)

    # Check for duplicate dimensions using set
    if len(normalized_dim) != len(set(normalized_dim)):
        raise ValueError(f"Duplicate dimensions found in dim list: {dim}")

    sorted_dim = sorted(normalized_dim)

    dst_shape = [core._constexpr_to_value(x) for x in input.shape]
    ret_ty = core.block_type(input.type.scalar, dst_shape)

    return core.tensor(_builder.create_flip(input.handle, sorted_dim), ret_ty)
