...
class _softmax(torch.autograd.Function):
    ...
    def forward(ctx, a, scale, rel_logits, is_causal, spdims, block, lut, maxlut, is_dense):
        ...
        _blocksparse_softmax_fwd[grid](
            out, a, a.stride(0), lut, rel_logits, rel_shape[-1], rel_strides[0], rel_strides[1],  # relative attn
            scale, is_causal, BLOCK_SIZE=block, ROW_SIZE=next_power_of_2(maxlut), IS_DENSE=is_dense,
    ...
    def backward(ctx, dout):
        ...
        _blocksparse_softmax_bwd[grid](da, da.stride(0), dout, dout.stride(0), out, out.stride(0), ctx.scale, lut, dr,
                                       ctx.rel_shape[-1], ctx.rel_strides[0], ctx.rel_strides[1], ctx.rel_strides[2],
                                       ctx.is_causal, BLOCK_SIZE=ctx.block, ROW_SIZE=next_power_of_2(ctx.maxlut),
                                       IS_DENSE=ctx.is_dense, num_warps=num_warps(ctx.maxlut))
