 ...
 */
 ...
 typedef enum HostLogLevel { LOG_DEBUG, LOG_INFO, LOG_WARNING, LOG_ERROR, LOG_FATAL, LOG_MAX } HostLogLevel;
 ...
 class HrtBootParam {
    public:
     HrtBootParam(uint32_t i_num, uint32_t o_num, uint32_t p_num) : i_num(i_num), o_num(o_num), p_num(p_num) {
         uint32_t bufsize = (sizeof(BootParamHead) + (i_num + o_num + 1) * sizeof(BootParamDyninfo));
     ...
     void set_dev_input_tensor(uint32_t idx, std::shared_ptr<tensor_info_t> tensor);
 ...
 class TsmModel {
    public:
     ...
     std::string case_name;
     std::string case_dir;
     std::shared_ptr<HostParamElem> so_list[MAX_MODEL_NUM][TILE_MAX_NUM];  // 编译出的so文件
 ...
 std::shared_ptr<json_common_info_multi_card_t> get_multi_card_common_info_from_file(std::string file);
 std::string get_docker_verison();
 TSM_RETCODE set_multi_graph(TsmModel *&kmodel, std::shared_ptr<HrtBootParam> &hostboot,
                             const TsmDevicePtr &dev_dyn_mods_ptr, const TsmDevicePtr &dev_tlv_ptr, TsmDevicePtr ext_ptr);
