...
TSM_RETCODE TsmSetDevice(uint32_t first_phy_id, uint32_t card_x, uint32_t card_y, std::vector<TsmDevice *> &devs);
TSM_RETCODE TsmSetDeviceOld(uint32_t chip_id, TsmDevice *dev); /* 该接口为提供给MLIR的过度版本，其他组件不要调用 */
...
TSM_RETCODE TsmCompile(std::vector<TsmDevice *> devs, TsmModel &kmodel, std::string option, CompileOption compl_op);
TSM_RETCODE TsmCompileMultiGraph(std::vector<TsmDevice *> devs, TsmModel &kmodel, std::string option,
                                 CompileOption compl_op);
TSM_RETCODE TsmLaunch(TsmDevice *dev, TsmModel &kmodel);
TSM_RETCODE TsmLoadKernel(TsmDevice *dev, std::vector<TsmModel *> &kmodel_vec, char *module_symbol);
TSM_RETCODE TsmUnloadKernel(TsmDevice *dev, std::vector<TsmModel *> &kmodel_vec);
TSM_RETCODE TsmRun(TsmDevice *dev, TsmDevicePtr bootpm_dev);
TSM_RETCODE TsmAsyncRun(tsmStream_t stream, TsmDevice *dev, TsmDevicePtr bootpm_dev);
...
TSM_RETCODE TsmMemcpyH2D(TsmDevicePtr dst, const void *src, uint64_t byte_count);
TSM_RETCODE TsmMemcpyD2H(const void *dst, TsmDevicePtr src, uint64_t byte_count);
TSM_RETCODE TsmMemcpyOffsetH2D(TsmDevicePtr dst, const void *src, uint64_t offset, uint64_t byte_count);
TSM_RETCODE TsmMemcpyOffsetD2H(const void *dst, TsmDevicePtr src, uint64_t offset, uint64_t byte_count);
TSM_RETCODE TsmMemcpyD2D(const void *dst, TsmDevice *dst_dev, const void *src, TsmDevice *src_dev, uint64_t byte_count);
TSM_RETCODE TsmSend(const void* sendbuff, size_t count, txcclDataType_t datatype, TsmDevice *dev, int peer, txcclComm_t comm, tsmStream_t stream);
TSM_RETCODE TsmRecv(void* recvbuff, size_t count, txcclDataType_t datatype, TsmDevice *dev, int peer, txcclComm_t comm, tsmStream_t stream);
TSM_RETCODE TsmResetDevice(TsmDevice *dev);
TSM_RETCODE TsmReleaseDevice(TsmDevice *dev);
TSM_RETCODE TsmMemGetInfo(TsmDevicePtr ptr, uint32_t &card_id, uint64_t &addr, uint64_t &size);
...
TSM_RETCODE TsmDeviceSerialize(const TsmDevice *const &dev, void *&buffer, size_t &size);
TSM_RETCODE TsmDeviceDeSerialize(TsmDevice *&dev, const void *const &buffer);
TSM_RETCODE TsmSetMonitorInfo(TsmDevice *dev);
TSM_RETCODE TsmProcessProfData(TsmDevice *dev, TsmProfAction prof_action, uint16_t prof_type);
TSM_RETCODE TsmHostH2D(TsmDevice *dev, uint64_t input_host_addr, uint64_t input_size, int32_t index);
TSM_RETCODE TsmHostFlush(TsmDevice *dev, uint64_t boot_param_ptr, uint8_t *host_buffer, size_t size);
 ...
 * #define rt_log(level, format, ...) tsm_log(__FILE__, __func__, __LINE__, TSM_RUNTIME, level, format, ##__VA_ARGS__)
 *
 * void func() {
 *   rt_log(LOG_DEBUG, "....\n");
 *   rt_log(LOG_INFO, "....\n");
 ...
 *   rt_log(LOG_ERROR, "....\n");
 * }
 * 默认日志级别为INFO，通过设置 HOST_LOG_LEVEL 更改日志级别，一般就设置成INFO和DEBUG。
 * 注意：
 *    其中rt_log为各组件定制名称，切勿重复，TSM_RUNTIME表示模块ID，各模块到hrt_common.h找到自己的宏，没有的可以联系runtime来增加。
 */
void tsm_log(const char *file_name, const char *func_name, uint32_t line_number, TsmModuleType module_type,
             HostLogLevel level, const char *format, ...);
#endif