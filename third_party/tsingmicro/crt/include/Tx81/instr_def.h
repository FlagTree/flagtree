...
typedef struct Ncc_CT_GR_Ctl_Regs {
  ...
  uint8_t rnd_mode;  // 0 :round to nearest even , 1 :round to zero, 2 :round to
                     // positive infinity, 3 :round to negative infinity, 4
                     // :stochastic round
  uint8_t
...
typedef struct Ncc_CT_GR_Param_Regs {
  ...
  uint64_t
      wb_data1; // The pointer of Return value. [32] DATA_VALID, [31:0] data,
                // 函数有两个返回值时，第二个返回数据写在此寄存器，当只有一个返回值时，此寄存器无效
  uint32_t src0_end; // spm地址(src0结束地址), xxx_end = src/dst +
                     // 对应操作数在spm中存储范围
  uint32_t src1_end;
  uint32_t dst0_end;
  uint32_t dst1_end;
...
typedef struct Ncc_NE_GR_Param_Regs {
  ...
  uint64_t tfr_0;   // src0 nhwc, [15:0]tensor
                    // batch/h/w(范围1~4096);tensor通道数(范围1~16384)
  uint64_t tfr_1;   // conv: out nhwc, 同上tfr_0
  uint64_t pdr;     // pad [15:0]top bottom left right,
                    // 分别是上下左右pad的行/列数(范围0~1023)
  uint64_t unpdr;   // unpad [15:0]top bottom left right
  uint64_t
      swr; // [15:0]Kx(范围1~255) Ky(范围1~255) Sx(范围1~1023) Sy(范围1~1023)
  uint64_t dilation; // [15:0]空洞卷积的x方向大小(范围1-1023),
                     // [15:0]空洞卷积的y方向大小(范围1-1023)
  ...
  */
  ...
  uint8_t quant_zp_pre;   // 输入零点(0-255), [23:16] conv:act_zp
                          // gemm:left_zp(范围：0-255)
          ...
          for(j = 0; j < itera1; j++)
              for(k = 0; k < itera0; k++)
                  for(l = 0; l < elem_count; l++)
                      dst[l + elem_coun * k + elem_coun * src_itera0 * j +
     elem_coun * src_itera0 * src_itera1 * i] = \ src[l + k * src_stride0 + j *
     src_stride1 + i * src_stride2];
  */
...
typedef struct Ncc_TDMA_GR_Param_Regs {
  ...
  uint32_t elem_count; // vector操作的元素个数.
                       // memset、gatherscatter指令中代表byte number
  /*
...
typedef struct Ncc_CSR_GR_SERIAL_MODE {
  uint8_t
      serial_mode; // [0] 1：串行模式，所有指令进一个queue，不做指令间并行
                   // 0：并行模式，指令按照指令类型进独立的queue，检测依赖乱序发射
                   // (测试默认为1，正式版本修改为0)
...
typedef struct Ncc_DTE_GR_Param_Regs {
  ...
  uint32_t mode;        // [1:0] mode:0->gather/scatter, unicast, 1-> scatter,
                        // broadcast, 3-> shuffle(3D gather). [8:8] sg_flag:
                        // 0->scatter, 1->gather, [16:16] dim_flag: only unicast
                        // mode(=0), 0->1D transport, 1->2D transport
  uint32_t length;      // count data bytes
  uint8_t dest_num; // if mode[0:0] is 0, then it's value is 1; otherwise it's
                    // value is between 1 and 31.
  uint32_t
      stride0; // if mode[0:0] is 0, then stride can be setted, unit is byte.
  ...
  uint32_t iteration2;
  uint16_t
      max_axi_num;   // [7:0] axi_write_outstanding, [15:8] aix_read_outstanding
  uint8_t cmd_valid; // 1: activate dma, 0: no action.
  // uint8_t dma_status; // [0:0] 0->unfinished, 1->finished. [8:8] 0/1, record
  // the error of AXI bus or other DMA transmission.
  uint16_t mem_burstlen;    // [7:0] mem_burst_len_write, default value: 0x10;
                            // [15:8] mem_burst_len_read, default value: 0x10
...
typedef struct NCC_CSR {
  uint64_t ib_status; //[7:0]IB_COUNTER: 指令buffer剩余指令数目, [8]TASK_DONE,
                      // 1：task执行结束, 0：task 正在执行, [63:9]Reserved
  uint64_t
  ...
  uint64_t
      serial_mode; //[0]SERIAL_MODE, 1：串行模式，0：并行模式, [63:1]Reserved
} NCC_CSR;
