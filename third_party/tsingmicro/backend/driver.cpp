...
TSM_RETCODE kernel_result_process(std::vector<TsmDevice *> devices,
                                  uint32_t dev_index,
  ...
  for (size_t i = 0; i < chip_info->output_num; ++i) {
    ...
    std::memcpy(chip_info->output[i]->shape, out_tensor->shape,
                sizeof(out_tensor->shape));
    chip_info->output_size[i] =
        hrt_get_dtype_size((DTYPE)chip_info->output[i]->dtype);
...
static void setHostBoot(std::shared_ptr<chip_common_info_t> &chip_info,
  ...
  for (size_t i = 0; i < chip_info->input_dev_addr.size(); ++i) {
    hostboot->set_dev_input(i, chip_info->input_dev_addr[i],
                            chip_info->input_size[i]);
    hostboot->set_dev_input_tensor(i, chip_info->input[i]);
  }

  for (size_t i = 0; i < chip_info->output_dev_addr.size(); ++i) {
    hostboot->set_dev_output(i, chip_info->output_dev_addr[i],
                             chip_info->output_size[i]);
  }

  for (size_t i = 0; i < chip_info->param_num; ++i) {
    hostboot->set_dev_param(i, chip_info->param_dev_addr[i],
                            chip_info->param_size[i]);
  }
...
static bool init_tx81_runtime_if_needed() {
  ...
  if (TsmGetDeviceNum(device_num) != RET_SUCCESS || device_num == 0) {
    PyErr_SetString(PyExc_RuntimeError,
                    "Failed to get Tx81 device count or no devices found");
    TsmDeInitRuntime();
    return false;
  }
  ...
  if (TsmSetDevice(first_phy_id, card_x, card_y, g_tx81_devices) !=
      RET_SUCCESS) {
...
static PyObject *launch(PyObject *self, PyObject *args) {
  ...
  // 需要的输入信息： devices, case_dir(按固定路径存放的kernelso)，
  // input_host_addr/input_size/input_num,
  ...
  compl_option.enable_kcore_so = 1;
  new_model->case_dir =
      "/tmp/todo"; // 参数传入， kernelso路径，同streambin/kcorebin文件夹路径

  if (TsmCompileMultiGraph(devices, *new_model, option, compl_option) !=
      RET_SUCCESS) {
  ...
  std::shared_ptr<HrtBootParam> hostboot =
      std::make_shared<HrtBootParam>(input_num, output_num, param_num);
  ...
  for (uint32_t i = 0; i < chip_info->input_num; ++i) {
    ...
    chip_info->input_host_addr = std::vector<uint64_t>{
        0x0, 0x0, 0x0, 0x0, 0x0, 0x0}; // TODO: 填写实际输入地址
  }

  for (uint32_t i = 0; i < chip_info->output_num; ++i) {
    chip_info->output_size[i] = 1; // TODO：填写实际输出大小
    chip_info->output_host_addr =
        std::vector<uint64_t>{0x0}; // TODO: 填写实际输出地址
  }
  ...
  for (uint32_t dev_index = 0; dev_index < devices.size(); ++dev_index) {
    ...
    if (TsmDeviceMalloc(devices[dev_index], dev_dyn_mods_ptr, dyn_mod_size) !=
        RET_SUCCESS) {
      return NULL;
    }
    TsmDevicePtr dev_tlv_ptr;
    if (TsmDeviceMalloc(devices[dev_index], dev_tlv_ptr,
                        sizeof(DynTLV_DynMods)) != RET_SUCCESS) {
      return NULL;
    }

    TsmDevicePtr dev_kernel_head_ptr;
    if (TsmDeviceMalloc(devices[dev_index], dev_kernel_head_ptr,
                        sizeof(Kernel_Head)) != RET_SUCCESS) {
      return NULL;
    }
    TsmDevicePtr dev_kernel_param_ptr;
    if (TsmDeviceMalloc(devices[dev_index], dev_kernel_param_ptr,
                        sizeof(Kernel_Param)) != RET_SUCCESS) {
      return NULL;
    }

    Kernel_Head *host_kernel_head_ptr =
        (Kernel_Head *)malloc(sizeof(Kernel_Head));
    Kernel_Param *host_kernel_param_ptr =
        (Kernel_Param *)malloc(sizeof(Kernel_Param));

    host_kernel_head_ptr->param_type = 1;
    host_kernel_head_ptr->param_num = 1; // Number of kernel arguments
    host_kernel_head_ptr->param_addr =
        dev_kernel_param_ptr; // 将kernel 使用的参数地址赋值
    ...
    TsmMemcpyH2D(dev_kernel_head_ptr, host_kernel_head_ptr,
                 sizeof(Kernel_Head));
    TsmMemcpyH2D(dev_kernel_param_ptr, host_kernel_param_ptr,
                 sizeof(Kernel_Param));
    ...
    set_multi_graph(kmodel_vec[0], hostboot, dev_dyn_mods_ptr, dev_tlv_ptr,
                    dev_kernel_head_ptr);

    TsmDevicePtr bootpm_dev;
    if (TsmDeviceMalloc(devices[dev_index], bootpm_dev,
                        hostboot->get_maxlen()) != RET_SUCCESS) {
      return NULL;
    }
    if (TsmMemcpyH2D(bootpm_dev, hostboot->get_bootpmbuffer(),
                     hostboot->get_maxlen()) != RET_SUCCESS) {
      return NULL;
    }
    ...
    if (kernel_result_process(devices, dev_index, hostboot, chip_info,
                              bootpm_dev, new_model->case_dir) != RET_SUCCESS) {
